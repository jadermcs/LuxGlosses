...
try:
    from src.sensealign.utils.french_wordnet import WordNet
except ImportError:
    from utils.french_wordnet import WordNet
...
def main():
    ...
    data.dropna(subset=['wn_definition']).to_csv("output/multilingual_definitions.csv", sep="\t", index=False)
