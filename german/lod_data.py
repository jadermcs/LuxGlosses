...
for entry in root:
    ...
    for meaning in entry.findall("./microStructure/grammaticalUnit/meaning"):
        word = meaning.find("./targetLanguage[@lang='de']/translation")
        clar = meaning.find("./targetLanguage[@lang='de']/semanticClarifier")
        if word is not None:
            word = word.text
        if clar is not None:
            clar = clar.text
...
df.to_csv("data/lod_german_word.csv", sep="\t", index=False)
